...
%
% Calculate the volume of importtradevolumes
% [It, Dt] = importtradevolumes(U, Y, dim)
%
% returns the import trade volumes
% It = matrix with intermediate import trade, dimension dim x dim
% Dt = matrix with direct import trade, dimension dim x dim
% The diagonal elements are filled with 0
%
% Need in workspace: 
...
%
function [It, Dt] = importtradevolumes(U, Y, dim)
  ...
  for i = 0:dim-1
    for j = 0:dim-1
      ...
      if (i != j)
        ...
        It(i+1,j+1) = sum(sum(U(p_idx0:p_idx1,i_idx0:i_idx1)));
        Dt(i+1,j+1) = sum(sum(Y(p_idx0:p_idx1,y_idx0:y_idx1)));
